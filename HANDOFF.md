...
### Сесія 2026-05-29 — UI-фікси та курси валют

| PR / коміт | Що зроблено |
|-----------|-------------|
| PR #27 | **Sidebar бейдж**: число відцентровано (`leading-none` + `tabular-nums`) — більше не з'їжджає. **Курси валют**: `1 EUR = 29 CZK`, `1 USD = 25 CZK` + узгоджені зворотні курси в `lib/calculations.ts` (`EXCHANGE_RATES`). **Технічний лист клієнту**: прибрано `hideSheet` у `NabidkaPublic` → клієнт бачить "Stáhnout technický list" у модалці деталей продукту. **Картка товару**: відступ + роздільник над CTA "Přidat do nabídky", коли продукт без технічного листа (`ProductDetailModal`) |
